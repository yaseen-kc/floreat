/**
 * Canonical Roof request contract — the single source of truth shared by the
 * backend (request validation) and the frontend (form validation).
 *
 * This is the LOOSE contract: only the core dimensions are required; every
 * structural section field is optional, matching what the API accepts. The
 * frontend layers a stricter variant on top (see frontend roof.schema.ts) that
 * makes the section fields required to force Step 2 completion — that is an
 * additive `.required()`/`.partial()` layer, NOT a fork of this definition.
 *
...
/* ──────────────────────────────────────────────────────────────────────────
 * Roof create/upsert payload (loose contract).
 * ────────────────────────────────────────────────────────────────────────── */
...
export const createRoofSchema = z.object({
  ...
  // `sideColumnsWidthHeight` is a DERIVED value, never a user input. The
  // frontend computes it for live preview; the backend recomputes it
  // authoritatively in roof.service before persisting (see deriveSideColumnsWidthHeight).
  // It is accepted but optional here, and any client-supplied value is overwritten server-side.
...
/** Validated payload for creating/upserting a roof (loose contract). */
