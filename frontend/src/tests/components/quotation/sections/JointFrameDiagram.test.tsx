...
const frame: FrameDiagram = {
  ...
  nodes: [
    ...
    { x: 100, y: 100, label: 'A-1', group: 'roof', jointId: 'A_1' },
    { x: 150, y: 150, label: 'FB-4', group: 'foundation', jointId: 'FB4' },
  ],
}
...
describe('JointFrameDiagram', () => {
  it('renders a labelled, focusable node per joint', () => {
    ...
    expect(screen.getByText('FB-4')).toBeInTheDocument()
    expect(screen.getByRole('button', { name: 'Joint A-1' })).toBeInTheDocument()
  })

  it('reports the store group + enum jointId on click', async () => {
    const onSelect = vi.fn()
    render(<JointFrameDiagram frame={frame} onSelectJoint={onSelect} />)
    await userEvent.click(screen.getByRole('button', { name: 'Joint A-1' }))
  ...
  it('activates a node with the keyboard', async () => {
    const onSelect = vi.fn()
    render(<JointFrameDiagram frame={frame} onSelectJoint={onSelect} />)
    screen.getByRole('button', { name: 'Joint FB-4' }).focus()
