...
describe('WizardActionBar Step 3 mezzanine persistence', () => {
  ...
  it('upserts an empty payload for a blank draft and still advances', async () => {
    ...
    await waitFor(() => expect(useQuotationStore.getState().currentStep).toBe(4))
    expect(mocks.upsertMezzMutateAsync).toHaveBeenCalledWith({ jobId: 'job-1', payload: {} })
  })
...
describe('WizardActionBar Step 4 stair persistence', () => {
  ...
  it('upserts an empty payload for a blank draft and still advances', async () => {
    ...
    await waitFor(() => expect(useQuotationStore.getState().currentStep).toBe(5))
    expect(mocks.upsertStairMutateAsync).toHaveBeenCalledWith({ jobId: 'job-1', payload: {} })
  })
...
describe('WizardActionBar Step 9 spec advance', () => {
  beforeEach(() => {
    localStorage.clear()
    useQuotationStore.getState().resetQuotation()
    mocks.navigate.mockReset()
  ...
  it('upserts the spec and advances to step 10 on Continue', async () => {
    ...
    await waitFor(() => expect(useQuotationStore.getState().currentStep).toBe(10))
  ...
  it('stays on step 9 and does not advance when the spec upsert fails', async () => {
