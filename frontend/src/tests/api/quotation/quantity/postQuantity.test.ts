...
import { upsertQuantity, useUpsertQuantity } from '@/api/quotation/quantity/postQuantity'
...
describe('upsertQuantity', () => {
  ...
  it('calls apiFetch with POST method, nested job quantity path, token, and payload', async () => {
    mockedApiFetch.mockResolvedValueOnce(quantity)

    const result = await upsertQuantity('token-123', 'job-1', payload)
  ...
  it('propagates errors thrown by the API layer', async () => {
    ...
    await expect(upsertQuantity('token', 'job-1', payload)).rejects.toThrow('API error: 400')
  })
})

describe('useUpsertQuantity', () => {
  beforeEach(() => {
    mockedApiFetch.mockReset()
  })

  it('upserts the quantity and invalidates detail and lists caches', async () => {
    mockedApiFetch.mockResolvedValueOnce(quantity)
    const { wrapper, queryClient } = makeWrapper()
    const invalidateSpy = vi.spyOn(queryClient, 'invalidateQueries')

    const { result } = renderHook(() => useUpsertQuantity(), { wrapper })
