...
import { upsertCanopy, useUpsertCanopy } from '@/api/quotation/canopy/postCanopy'
import type { CreateCanopyPayload } from '@/api/quotation/canopy/postCanopy'
...
describe('upsertCanopy', () => {
  ...
  it('calls apiFetch with POST method, nested job canopy path, and token', async () => {
    mockedApiFetch.mockResolvedValueOnce(canopy)

    const result = await upsertCanopy('token-123', 'job-1', payload)
  ...
  it('forwards a null token', async () => {
    ...
    await upsertCanopy(null, 'job-1', payload)
  ...
  it('propagates errors thrown by the API layer', async () => {
    ...
    await expect(upsertCanopy('token', 'job-1', payload)).rejects.toThrow('API error: 400')
  })
})
...
describe('useUpsertCanopy', () => {
  beforeEach(() => {
    mockedApiFetch.mockReset()
  })

  it('upserts the canopy and returns the typed canopy', async () => {
    mockedApiFetch.mockResolvedValueOnce(canopy)
    const { wrapper } = makeWrapper()

    const { result } = renderHook(() => useUpsertCanopy(), { wrapper })
