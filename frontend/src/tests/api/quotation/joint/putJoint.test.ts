...
import { updateJoint, useUpdateJoint } from '@/api/quotation/joint/putJoint'
import type { UpdateJointPayload } from '@/api/quotation/joint/putJoint'
import type { Joint } from '@/api/quotation/joint/getJoint'
import { jointKeys } from '@/api/quotation/joint/queryKeys'

const mockedApiFetch = vi.mocked(apiFetch)

const payload: UpdateJointPayload = {
  secondaryBeamsNumberOfBolts: 6,
  jointBoltRoof: [{ roofJointId: 'B_1', boltDiameter: 16 }],
}
...
describe('updateJoint', () => {
  ...
  it('calls apiFetch with PUT method, nested job joint path, and token', async () => {
    mockedApiFetch.mockResolvedValueOnce(joint)

    const result = await updateJoint('token-123', 'job-1', payload)
  ...
  it('forwards a null token', async () => {
    ...
    await updateJoint(null, 'job-1', payload)
  ...
  it('propagates errors thrown by the API layer', async () => {
    ...
    await expect(updateJoint('token', 'job-1', payload)).rejects.toThrow('API error: 404')
  })
})
...
describe('useUpdateJoint', () => {
  beforeEach(() => {
    mockedApiFetch.mockReset()
  })

  it('updates the joint and returns the typed joint', async () => {
    mockedApiFetch.mockResolvedValueOnce(joint)
    const { wrapper } = makeWrapper()

    const { result } = renderHook(() => useUpdateJoint(), { wrapper })
